...
std::optional<emp::BitArray<100>> GetNecessaryInstructions(CPU org_cpu, size_t test_task_id) {
  emp::Random random(-1);
  const Genome control_program = org_cpu.genome;
  Genome test_program = control_program;
  ...
  if (!can_do_task) {
    ...
    for (size_t k = 0; k <= control_program.size() - 1; k++) {
      test_program[k].op = Operation::Nop;
      // Passing nullptr for random is fine as long as we don't call Mutate()
      CPU temp_cpu = CPU(org_cpu.state.host, org_cpu.state.world, nullptr, test_program);
